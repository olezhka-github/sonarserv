...
if not all([SENDER_EMAIL, RESEND_API_KEY]):
...
def send_verification_email(email: str, token: str):
    ...
    try:
        ...
        # Визначаємо ім'я відправника (можна взяти з SENDER_EMAIL або використати домен)
        from_email = SENDER_EMAIL
        # Якщо SENDER_EMAIL в форматі "email@example.com", можна додати ім'я
        sender_name = "SonarServ"
        from_header = f"{sender_name} <{from_email}>"
        
        # Створюємо HTML та текст версії листа
        html_content = f"""
        <html>
...
"""
        ...
        # Параметри для відправки через Resend
        params = {
            "from": from_header,
            "to": [email],
            ...
            "html": html_content,
            "text": text_content,
            "reply_to": SENDER_email,  # Додаємо Reply-To
            "headers": {
        ...
        # Не кидаємо помилку далі, щоб не заважати реєстрації користувача
        # Можна додати fallback механізм або просто логувати помилку
